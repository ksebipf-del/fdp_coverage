...
class FdpCoverageToolDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        # Neuer Button: Export nach Optimierung
    ...
    def export_optimized_to_excel(self):
        """
        Exportiert Optimized_FDP_Points + New_FDP_Points (falls vorhanden) nach Excel mit:
        - Kept FDPs: Original-Daten (FDP, Admin_Level_2, Admin_Level_3, Lat, Long)
        - Relocated FDPs: Original-Daten (FDP, Admin_Level_2, Admin_Level_3, neue Lat, Long)
        - New FDPs: generische Namen (FDP 1, 2, ...), Lat, Long
        - Für alle: Population Coverage (basierend auf aktueller Buffer-Größe)
        """
        from qgis.core import (
            QgsVectorLayer, QgsProject, QgsSpatialIndex, QgsGeometry, QgsPointXY,
        ...
        for layer in QgsProject.instance().mapLayers().values():
            if layer.name().startswith("WorldPop"):
                ...
                fdp_original_layer = layer
            elif layer.name() == "Optimized_FDP_Points":
                optimized_layer = layer
            elif layer.name() == "New_FDP_Points":
                new_points_layer = layer

        if not all([worldpop_layer, planning_layer, optimized_layer]):
            QMessageBox.critical(self, "Error", 
                "WorldPop, Planning Area and Optimized_FDP_Points must be loaded.\n"
                "Run optimization first (btnOptimizePoints).")
            return
        ...
        # 2. WorldPop clippen und nach UTM
        try:
            clipped_pop = processing.run("native:clip", {
                'INPUT': worldpop_layer,
                'OVERLAY': planning_layer,
        ...
        # Helper: Coverage für einen Punkt (UTM)
        def calc_coverage(point_geom_utm):
            buf = point_geom_utm.buffer(buffer_distance, 25)
            cov = 0
            for pid in pop_index.intersects(buf.boundingBox()):
                pf = pop_features.get(pid)
                if pf and buf.contains(pf['geom']):
                    cov += pf['pop']
            return cov

        # 3. Optimized FDPs verarbeiten
        rows = []

        # Optional: Original-FDP-Layer in UTM + SpatialIndex (für "fixed" Zuordnung)
        orig_utm = None
        orig_index = None
        if fdp_original_layer:
            try:
                orig_utm = processing.run("native:reprojectlayer", {
                    'INPUT': fdp_original_layer,
                    'TARGET_CRS': utm_crs,
                    'OUTPUT': 'TEMPORARY_OUTPUT'
                })['OUTPUT']
                orig_index = QgsSpatialIndex(orig_utm.getFeatures())
            except Exception:
                orig_utm = None
                orig_index = None

        # Transform UTM → WGS84
        xform_to_wgs = QgsCoordinateTransform(utm_crs, wgs84, QgsProject.instance().transformContext())

        # Reprojecte Optimized nach UTM
        try:
            opt_utm = processing.run("native:reprojectlayer", {
                'INPUT': optimized_layer,
                'TARGET_CRS': utm_crs,
                'OUTPUT': 'TEMPORARY_OUTPUT'
            })['OUTPUT']
        except Exception as e:
        ...
        # Hilfsfunktion: zu nächstem Original-FDP Attribute holen (nur für fixed)
        def nearest_orig_attrs(pt_utm):
            if not orig_utm or not orig_index:
                return {'FDP': '', 'Admin_Level_2': '', 'Admin_Level_3': ''}
            try:
                nn_ids = orig_index.nearestNeighbor(pt_utm, 1)
                if not nn_ids:
                    return {'FDP': '', 'Admin_Level_2': '', 'Admin_Level_3': ''}

                nn_id = nn_ids[0]
                nn_feat = next((f for f in orig_utm.getFeatures([nn_id])), None)
                if not nn_feat:
                    # Fallback ohne Filter
                    for f in orig_utm.getFeatures():
                        if f.id() == nn_id:
                            nn_feat = f
                            break
                if not nn_feat:
                ...
                return {
                    'FDP': self._feat_attr(nn_feat, 'FDP', ''),
                    'Admin_Level_2': self._feat_attr(nn_feat, 'Admin_Level_2', ''),
                    'Admin_Level_3': self._feat_attr(nn_feat, 'Admin_Level_3', '')
                }
            except Exception:
                return {'FDP': '', 'Admin_Level_2': '', 'Admin_Level_3': ''}

        for feat in opt_utm.getFeatures():
            # Typ: fixed/relocated
            ftype = self._feat_attr(feat, 'type', 'fixed')

            # Lat/Long (aktuelle Position)
            pt_utm = feat.geometry().asPoint()
            pt_wgs = xform_to_wgs.transform(QgsPointXY(pt_utm))

            # Coverage
            cov = calc_coverage(feat.geometry())

            # Attribute:
            if ftype == 'fixed':
                attrs = nearest_orig_attrs(pt_utm)
                fdp_name = attrs['FDP'] if attrs['FDP'] else 'FDP'
                admin2 = attrs['Admin_Level_2']
                admin3 = attrs['Admin_Level_3']
            else:
                # Für "relocated" keine Original-Namen
                fdp_name = ''
                admin2 = ''
                admin3 = ''
            ...
            rows.append({
                'FDP': fdp_name,
                'District (Admin_Level_2)': admin2,
                'TA (Admin_Level_3)': admin3,
                'Latitude': round(pt_wgs.y(), 6),
                'Longitude': round(pt_wgs.x(), 6),
                'Population Coverage': int(cov),
                'Type': ftype
            })

        # 4. New FDP Points (falls vorhanden)
        if new_points_layer:
            try:
                ...
                new_counter = 1
                for feat in new_utm.getFeatures():
                    pt_utm = feat.geometry().asPoint()
                    pt_wgs = xform_to_wgs.transform(QgsPointXY(pt_utm))
                    cov = calc_coverage(feat.geometry())

                    rows.append({
                        'FDP': f'FDP {new_counter}',
                        'District (Admin_Level_2)': '',
                        'TA (Admin_Level_3)': '',
                        'Latitude': round(pt_wgs.y(), 6),
                        'Longitude': round(pt_wgs.x(), 6),
                        'Population Coverage': int(cov),
                ...
                QMessageBox.warning(self, "Warning", f"Could not process New_FDP_Points:\n{str(e)}")

        # 5. DataFrame erstellen und nach Excel exportieren
        if not rows:
        ...
        try:
            df.to_excel(file_path, index=False, sheet_name='FDP_Coverage')
            QMessageBox.information(self, "Success",
